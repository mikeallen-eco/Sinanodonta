...
# load MaxEnt models
mod1 <- readRDS(paste0(outpath, "mod1.rds"))
mod2 <- readRDS(paste0(outpath, "mod2.rds"))
mod3 <- readRDS(paste0(outpath, "mod3.rds"))
mod4 <- readRDS(paste0(outpath, "mod4.rds"))
mod5 <- readRDS(paste0(outpath, "mod5.rds"))
...
png("figures/USA_predictions_5mod_avg.png", width = 5*400, height = 5*400, res = 400)
plot(mean_pred)
dev.off()
...
# writeRaster(mean_pred, paste0(outpath, "conus_predictions_5mod_mean.tif"), overwrite = T)

mean_pred <- rast(paste0(outpath, "conus_predictions_5mod_mean.tif"))
...
png("figures/DWS_predictions_5mod_avg.png", width = 4*400, height = 6*400, res = 400)
plot(mean_pred_dws)
