...
mod2 <- maxent(x=p.env2[,c(1:30)], ## env conditions
               p=p.env2$pa,   ## 1:presence or 0:absence
               path="/Users/mikea/Documents/research/Sinanodonta/output/maxent_out2", 
               args=c("responsecurves") ## parameter specification
)

mod3 <- maxent(x=p.env3[,c(1:30)], ## env conditions
               p=p.env3$pa,   ## 1:presence or 0:absence
               path="/Users/mikea/Documents/research/Sinanodonta/output/maxent_out3", 
               args=c("responsecurves") ## parameter specification
)

mod4 <- maxent(x=p.env4[,c(1:30)], ## env conditions
               p=p.env4$pa,   ## 1:presence or 0:absence
               path="/Users/mikea/Documents/research/Sinanodonta/output/maxent_out4", 
               args=c("responsecurves") ## parameter specification
)

mod5 <- maxent(x=p.env5[,c(1:30)], ## env conditions
               p=p.env5$pa,   ## 1:presence or 0:absence
               path="/Users/mikea/Documents/research/Sinanodonta/output/maxent_out5", 
               args=c("responsecurves") ## parameter specification
)

# save model output
saveRDS(mod1, paste0(outpath, "mod1.rds"))
saveRDS(mod2, paste0(outpath, "mod2.rds"))
saveRDS(mod3, paste0(outpath, "mod3.rds"))
saveRDS(mod4, paste0(outpath, "mod4.rds"))
saveRDS(mod5, paste0(outpath, "mod5.rds"))
...
png("figures/Europe_predictions_5mod_avg_redo.png", width = 5*400, height = 5*400, res = 400)
plot(mean_pred)
dev.off()
plot(mean_pred)


# writeRaster(mean_pred, paste0(outpath, "europe_predictions_5mod_mean.tif"), overwrite = T)
...
# write.csv(mod_AUC, "output/mod_AUC.csv")
